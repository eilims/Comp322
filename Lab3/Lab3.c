...
void filePermission(int userID, int userGroupID, int cpid,  char* fileName)
{
   cpid = fork();
   //Child process
   if (cpid == 0) 
...
int main(int argc, char** argv)
{
   ...
   int i;
   c = 1;
   //Push to pipe to force printing by one process at a time
   write(pd[1], &c, sizeof(c));
   ...
   if (argc == 1)
	...
	while((file = readdir(dir)) != NULL)
	  {
	     filePermission(userID, userGroupID, cpid, file->d_name);
	  }
   ...
   //Files are specified
	...
	for (i = 1; i < argc; i++)
	  {
	     filePermission(userID, userGroupID, cpid, argv[i]);
	  }
     }
